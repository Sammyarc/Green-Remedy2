    ...
    return (
        <div
            ...
            <div className="w-full md:w-3/4 bg-white shadow-lg rounded-lg">
                {
                    cart.length === 0
                        ? (<p className='font-Lora text-[5vw] md:text-[2vw] font-bold flex justify-center items-center my-[3vw] text-textColor'>Your cart is empty.</p>)
                        : (
                                        ...
                                        cart.map((item, index) => (
                                            <tr key={index} className="border-b">
                                                <td className="pl-4 py-4">
                                                    <FaTimes onClick={() => removeFromCart(item)} className="text-textColor text-[7vw] md:text-[1.2vw] cursor-pointer" />
                                                </td>
                                                <td className="px-2 py-4 flex items-center">
                                                    {/* Product Image Placeholder */}
                                                    <div className="w-16 h-16 bg-gray-200 mr-4"><img src={item.image} alt="Item" className="w-full h-full object-cover rounded-[0.5vw]" /></div>
                                                ...
                                                <td className="p-4 text-[7vw] md:text-[1vw] font-OpenSans">{`₦${ (item.price * item.quantity).toLocaleString()}`}</td>
                                            </tr>
                                        ))
                                    }
                                </tbody>
            ...
            <div
                ...
                <h3 className="font-Lora text-[5vw] md:text-[1.5vw] mb-4 bg-lightTextColor rounded-t-lg p-4 text-white">Cart Total</h3>
                ...
                <div className="flex justify-between mb-4 p-4">
                    <span className="font-bold text-[3.5vw] md:text-[1vw] font-OpenSans text-textColor">TOTAL</span>
