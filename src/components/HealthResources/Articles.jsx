...
const Articles = () => {
  return (
    <div className='flex flex-col gap-10 mt-20 font-Lora px-[6vw] md:px-[5vw] md:pt-[5vw] md:pb-[10vw]'>
      <header className='font-bold text-3xl text-center w-full text-textColor mb-8'>
        Explore Our Recent Article
      </header>
      <aside className='flex flex-col gap-36'>
        {articles.map((article) => {
          return (
            <div
              key={article.title}
              className='flex flex-col md:flex-row md:items-center md:gap-16 md:even:flex-row-reverse '
            >
              <div className='w-full md:w-[58%] lg:mr-10'>
                <img src={article.img} alt='Image' className='rounded-xl' />
              </div>
              <section className='w-full md:w-[42%]'>
                <h2 className='font-bold text-2xl text-textColor mb-5 mt-5 md:mt-0  lg:w-2/3'>
                  {article.title}
                </h2>
